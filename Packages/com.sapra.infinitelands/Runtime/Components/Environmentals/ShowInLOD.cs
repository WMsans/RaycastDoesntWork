...
namespace sapra.InfiniteLands{
    ...
    public class ShowInLOD : InfiniteLandsMonoBehaviour
    {
        ...
        void OnChunkChanges(CoordinateDataHolder data){
            if(data == null || (data != null && !infiniteLands.IsPointInChunk(transform.position, data.terrainConfiguration)))
                return;
        ...
        public override void Initialize(IControlTerrain lands)
        {
            ...
            if(store != null && Target != null){
                store.onProcessDone += OnChunkChanges;
                var current = store.GetHolderAt(transform.position);
                OnChunkChanges(current);
            }
        }
    }
}