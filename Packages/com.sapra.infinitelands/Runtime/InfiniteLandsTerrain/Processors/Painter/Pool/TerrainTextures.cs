...
namespace sapra.InfiniteLands
{
    public class TerrainTextures
    {
        ...
        public IHoldTextures[] AllTextures{ get; private set; }
        public Action OnTextureAssetModified;
        public bool AssetsAreDirty{ get; private set; }

        public TerrainTextures(IEnumerable<IHoldTextures> allTextures, Action<Texture2DArray> DestroyTextures, Shader forShader,
            TextureResolution textureResolution, float MeshScale)
        {
            var textureArrays = GetTextureArrays(forShader);

            AllTextures = allTextures.Distinct().ToArray();
            var allTextureData = AllTextures.SelectMany(a => a.GetTextures());
            ...
            foreach (var text in AllTextures)
            {
                if (text is UpdateableSO updateable)
                {
            ...
            GenerateTexturePool(texturePacks, AllTextures, DestroyTextures, textureResolution);
            ...
            IEnumerable<ITextureSettings> textureSettings = AllTextures.Select(a => a.GetSettings());
        ...
        void GenerateTexturePool(Dictionary<string, List<Texture2D>> _texturePacks, IHoldTextures[] _uniqueTextures, 
            Action<Texture2DArray> _destroyTextures, TextureResolution _textureResolution)
        {
            ...
            int defaultCount = _uniqueTextures.Length;
        ...
        public void Release()
        {
            foreach (IHoldTextures texture in AllTextures)
            {
                if (texture is UpdateableSO updateable)
                {
                    updateable.OnValuesUpdated -= TextureHasBeenModified;
                }
            }
