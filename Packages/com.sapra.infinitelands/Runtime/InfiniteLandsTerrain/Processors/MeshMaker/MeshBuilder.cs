...
namespace sapra.InfiniteLands.MeshProcess
{
    public static class MeshBuilder
    {
        ...
        private static JobHandle StaticMesh(MeshProcess meshProcess, UnityEngine.Mesh.MeshData meshData)
        {
            return StaticMeshJob.ScheduleParallel(meshData, meshProcess.meshSettings, meshProcess.worldFinalData, meshProcess.ObjectBounds);
        }

        public static UnityEngine.Mesh CreateMesh(){
            var current = new UnityEngine.Mesh();
            ReuseMesh(current);
            return current;
        }

        public static void ReuseMesh(UnityEngine.Mesh current){
            current.name = "Infinite Lands Mesh";
        }

        public static void Consolidate(MeshGenerationData data, List<UnityEngine.Mesh> selection)
        {
            for(int i = 0; i < data.meshDataArray.Length; i++){
                selection[i].bounds = data.generatedChunks[i].ObjectBounds;
            }
            UnityEngine.Mesh.ApplyAndDisposeWritableMeshData(data.meshDataArray, selection, NoCalculations());
        }
        ...
        public static void Configure(UnityEngine.Mesh.MeshData _meshData, int verticesLength, int indicesLength, Bounds _bounds){
