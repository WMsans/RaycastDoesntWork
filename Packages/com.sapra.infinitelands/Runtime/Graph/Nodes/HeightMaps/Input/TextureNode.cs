...
namespace sapra.InfiniteLands
{
    ...
    public class TextureNode : InfiniteLandsNode
    {
        ...
        protected override void Process(BranchData branch)
        {
            ...
            NativeArray<float3> vectorized = gridData.grid;
            ...
            var targetSpace = heightBranch.GetAllocationSpace(this, nameof(Output));

            var targetMap = map;

            Vector3 origin = new Vector3(Origin.x, 0, Origin.y);

            JobHandle job = TextureJob.ScheduleParallel(vectorized, targetMap, 
                textureAlphaData.values, resolution,
                branch.terrain.Position-origin, MinMaxHeight, targetSpace, branch.meshSettings.Resolution, Size,
                dependancy);
