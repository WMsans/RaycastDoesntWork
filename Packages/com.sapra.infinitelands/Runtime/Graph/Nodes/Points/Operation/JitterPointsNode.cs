...
namespace sapra.InfiniteLands
{
    ...
    public class JitterPointsNode : InfiniteLandsNode, IProcessPoints
    {
        ...
        {
            var reuser = new GenericNodeReuser<JitterPointsNode, JitterPointData>(branch, this);
            return AwaitableTools.WaitNode<JitterPointData, PointInstance, GenericNodeReuser<JitterPointsNode, JitterPointData>>(branch, ref reuser, out Output, this, nameof(Output));
        }
        ...
        private class JitterPointData : AwaitableData<PointInstance>, INodeReusable<JitterPointsNode>
        {
            public PointInstance Result{get; private set;}
            ...
            private ICreateGrid gridCreator;

            private int SubState;
            private bool Skip;

            AwaitableData<HeightData> awaitableData;
            MeshSettings meshSettings;

            public void Reuse(JitterPointsNode node, BranchData branch){
                Node = node;
                this.branch = branch;
                gridCreator = branch.GetData<GridBranch>().GridCreator;
                SubState = 0;
            ...
            public bool ProcessData()
            {
                ...
                if(SubState == 0){
                    ...
                    Node.TryGetInputData(branch, out previousPoints, nameof(Points));
                    SubState++;
                }

                if(SubState == 1){
                    if(!Node.IsAssigned(nameof(Position))){
                        Result = manager.GetPointInstance(Node, gridCreator, previousPoints.GridSize, previousPoints, branch.meshSettings.Seed);
                        SubState = 30;
                    }
                    else{
                        awaitableData = manager.ExtractHeightData(Node, nameof(Position), gridCreator, Vector3.zero, meshSettings, out newSettings);
                        SubState++;
                    }
                }
                ...
                if(SubState == 3){
                    if(!finalHeight.jobHandle.IsCompleted && !branch.treeData.ForceComplete) return false;
                    finalHeight.jobHandle.Complete();
                    
                    float MaxDistance = Mathf.Max(finalHeight.minMaxValue.y-finalHeight.minMaxValue.x, previousPoints.GridSize);
                    Result = manager.GetPointInstance(Node, gridCreator, MaxDistance, previousPoints, meshSettings.Seed);
