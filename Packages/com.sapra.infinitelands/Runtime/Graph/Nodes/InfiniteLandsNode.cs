...
{    
    ...
    public partial class InfiniteLandsNode
    {
        public readonly struct CachedPort
        {
            ...
            public CachedPort(PortData portData, PortData localPort, InfiniteLandsNode node)
            {
                originPort = portData;
                this.node = node;
        ...
        public void SetupNode(IGraph graph, string guid, Vector2 position){
            this.guid = guid;
        ...
        { //Getting and validating the connections
            ...
            foreach (FieldInfo field in cacheInputs)
            {
                ...
                { //No connection into that port{
                    var disabled = field.GetCustomAttribute<DisabledAttribute>();
                    if (disabled == null)
                    {
                        SetInvalid();
                        continue;
        ...
        /// <param name="fieldName"></param>
        /// <returns></returns>
        public bool ExtractNameNode(string inputFieldName, out string nodeOutputName, out InfiniteLandsNode ogNode)
        {
