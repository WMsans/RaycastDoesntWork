...
namespace sapra.InfiniteLands{
    public class PointManager
    {
        ...
        public PointInstance GetPointInstance(IProcessPoints processor, ICreateGrid createGrid,
            float gridSize, PointInstance dependancy, int seed, bool newPoints = false)
        {
            var factory = new PointInstanceFactory(processor, createGrid, seed, gridSize, newPoints, dependancy, this);
        ...
        public HeightAtPoint GetDataAtPoint(InfiniteLandsNode node, string fieldName, 
            ICreateGrid createGrid,
            Vector3 position, MeshSettings meshSettings)
        {
            
            var heightData = ExtractHeightData(node, fieldName,createGrid, position, meshSettings, out var newSettings);
            HeightAtPoint extractor = new HeightAtPoint(heightData, newSettings);
            return extractor;
        }

        public HeighDataExtractor ExtractHeightData(InfiniteLandsNode node, string fieldName, ICreateGrid createGrid, Vector3 position, MeshSettings meshSettings, out TreeData newSettings){
            return new HeighDataExtractor(node, fieldName, globalStore, createGrid, position, meshSettings, out newSettings);
        }
        ...
        private struct PointInstanceFactory : IFactory<PointInstance>
        {
            ...
            public PointInstanceFactory(IProcessPoints Processor, ICreateGrid createGrid, int seed, float gridSize, bool newPoints, PointInstance PreviousInstance, PointManager pointManager){
            ...
            {
                return new PointInstance(Processor, createGrid, gridSize, PreviousInstance, PointManager, seed, newPoints);
            }
        }
    }
}