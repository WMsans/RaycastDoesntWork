...
namespace sapra.InfiniteLands{
    public class PointInstance
    {
        ...
        private readonly ICreateGrid GridCreator;
        private readonly PointManager pointManager;

        private bool ReadOnlyInstance;

        public PointInstance(IProcessPoints Processor, ICreateGrid GridCreator, float gridSize, PointInstance PreviousInstance, PointManager pointManager, int seed, bool StoreNewPoints = false)
        {
        ...
        {
            return pointManager.GetDataAtPoint(node, fieldName, GridCreator, position, meshSettings);
        }
/* 
        public HeightAtPoint GetDataAtPoint(InfiniteLandsNode node, string fieldName, 
            Vector3 position, MeshSettings meshSettings)
