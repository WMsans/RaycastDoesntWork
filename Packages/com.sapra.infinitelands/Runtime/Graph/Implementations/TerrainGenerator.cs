...
namespace sapra.InfiniteLands
{
    ...
    {     
        ...
        public virtual IEnumerable<EdgeConnection> GetAllEdges() => edges;
        public virtual IEnumerable<InfiniteLandsNode> GetAllNodes() => nodes;
        ...
        public void ValidationCheck()
        {
            ...
            foreach (var node in nds)
            {
                var connections = edgs.Where(a => a.inputPort.nodeGuid.Equals(node.guid));
        ...
        public IEnumerable<ILoadAsset> GetAllAssetLoaders<M>() where M : IAsset => GetAllNodes().Where(a => a.isValid).OfType<ILoadAsset>();
        public IEnumerable<ILoadAsset> GetAsetLoaderOf<M>(M asset) where M : IAsset => GetAllAssetLoaders<M>().Where(a => a.AssetExists(asset));
        ...
        public static void PreconfigureDefaultGraph(TerrainGenerator world){
            HeightOutputNode output = world.CreateNode<HeightOutputNode>(Vector2.zero);
            ...
            simplex.Octaves = 5;
            world.output = output;
            EdgeConnection connection = new EdgeConnection(new PortData(simplex.guid, nameof(simplex.Output)), new PortData(output.guid, nameof(output.HeightMap))); 
