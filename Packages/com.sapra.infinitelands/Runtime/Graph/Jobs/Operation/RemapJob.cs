...
namespace sapra.InfiniteLands
{
    ...
    public struct RemapHeightJob : IJobFor
    {
        ...
        public void Execute(int i)
        {
            ...
            float value = heightMap[current.IndexOffset + index];
            float normalized = JobExtensions.invLerp(CurrentMinMax.x, CurrentMinMax.y, value);
            heightMap[target.IndexOffset + i] = lerp(TargetMinMax.x, TargetMinMax.y, normalized);
        }
    ...
    public struct RemapCurveJob : IJobFor
    {
        ...
        public void Execute(int i)
        {
            ...
            float value = heightMap[current.IndexOffset + index];
            //float normalized = JobExtensions.invLerp(CurrentMinMax.x, CurrentMinMax.y, value);
            heightMap[target.IndexOffset + i] = curve.EvaluateLerp(value, CurrentMinMax.x, CurrentMinMax.y, CurrentMinMax.x, CurrentMinMax.y);//               lerp(CurrentMinMax.x, CurrentMinMax.y, saturate(curve.EvaluateLerp(value, CurrentMinMax.x, CurrentMinMax.y)));
        }
