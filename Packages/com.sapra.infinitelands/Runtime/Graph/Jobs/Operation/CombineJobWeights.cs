    ...
    public struct AddJobWeights : IJobFor
    {
        ...
        public void Execute(int i)
        {
            ...
            for (int x = 0; x < arrayCount; x++)
            {
                ...
                sum += globalArray[indexAndRe.IndexOffset + locator];

                var targetWeight = targetWeights[x];
                int weightLocator = MapTools.RemapIndex(i, target.Resolution, targetWeight.Resolution);
                globalArray[targetWeight.IndexOffset + weightLocator] = 1;

            }

            globalArray[target.IndexOffset + i] = sum;
        }
    ...
    public struct MaxJobWeights : IJobFor
    {
        ...
        public void Execute(int i)
        {
            ...
            for (int x = 0; x < arrayCount; x++)
            {
                var indexAndRe = indices[x].indexData;
                int locator = MapTools.RemapIndex(i, target.Resolution, indexAndRe.Resolution);
                sum = max(globalArray[indexAndRe.IndexOffset + locator], sum);
            }
    ...
    public struct MinJobWeights : IJobFor
    {
        ...
        public void Execute(int i)
        {
            ...
            for (int x = 0; x < arrayCount; x++)
            {
                var indexAndRe = indices[x].indexData;
                int locator = MapTools.RemapIndex(i, target.Resolution, indexAndRe.Resolution);
                sum = min(globalArray[indexAndRe.IndexOffset + locator], sum);
            }
            JobExtensions.ApplyFoundValue(i, sum, arrayCount, indices, globalArray, targetWeights, target);        
            globalArray[target.IndexOffset + i] = sum;
        }
    ...
    public struct HeightBlendWeights : IJobFor
    {
        ...
        public void Execute(int i)
        {
            ...
            for (int x = 0; x < arrayCount; x++)
            {
                var indexAndRe = indices[x].indexData;
                int locator = MapTools.RemapIndex(i, target.Resolution, indexAndRe.Resolution);
                remapedWeights[x] = JobExtensions.invLerp(MinMax.x, MinMax.y, globalArray[indexAndRe.IndexOffset + locator]);
            ...
            for (int x = 0; x < arrayCount; x++)
            {
                ...
                int locator = MapTools.RemapIndex(i, target.Resolution, indexAndRe.Resolution);
                sum += globalArray[indexAndRe.IndexOffset + locator] * density;

                var targetWeight = targetWeights[x];
                int weightLocator = MapTools.RemapIndex(i, target.Resolution, targetWeight.Resolution);
                globalArray[targetWeight.IndexOffset + weightLocator] = density;
            }
    ...
    public struct NormalizedMultiplyJobWeights : IJobFor
    {
        ...
        public void Execute(int i)
        {
            ...
            for (int x = 0; x < arrayCount; x++)
            {
                ...
                int locator = MapTools.RemapIndex(i, target.Resolution, indexAndRe.Resolution);
                mult *= JobExtensions.invLerp(minMax.x, minMax.y,globalArray[indexAndRe.IndexOffset + locator]);
            ...
            globalArray[target.IndexOffset + i] = mult;
        }
