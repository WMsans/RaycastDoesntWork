 ...
 */
        ...
        public float3 calculateNormalAtIndexFast(float2 pos)
        {
            ...
            float heightNW = globalMap[index + current.IndexOffset];
            float heightNE = globalMap[index + current.IndexOffset + 1];
            float heightSW = globalMap[index + current.IndexOffset + current.Resolution + 1];
            float heightSE = globalMap[index + current.IndexOffset + current.Resolution + 1 + 1];
        ...
        public float3 position(int x, int y)
        {
            ...
            float3 position = float3(ps.x, globalMap[index + current.IndexOffset], ps.z);
            return position;
        }
        ...
        {
            int index = MapTools.GetFlatIndex(int2(x, y), normalIndex.Resolution, current.Resolution);
            return globalMap[index + current.IndexOffset];
        }
    }
    ...
    public struct GenerateNormalMap : IJobFor
    {
        ...
        public static JobHandle ScheduleParallel(NativeArray<float3> vertices, NativeArray<float3> normalMap,
            NativeArray<float> globalMap, IndexAndResolution normalIndex, IndexAndResolution original, int verticesResolution,
