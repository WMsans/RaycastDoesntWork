...
namespace sapra.InfiniteLands{
    ...
    public struct FlattenAtPointsJob : IJobFor
    {
        ...
        public void Execute(int i)
        {
            ...
            float value = heightMap[current.IndexOffset + currentIndex];
            float height = 0;
            float totalWeight = 0;
            ...
            heightMap[target.IndexOffset + i] = math.lerp(value, averageHeight, math.saturate(totalWeight));
        }

        public static JobHandle ScheduleParallel(NativeArray<float> globalMap, NativeArray<float3> vertices, NativeArray<PointTransform> points,
            IndexAndResolution current, IndexAndResolution target,
            int PointsCount, int VerticesResolution, float Strength, float Distance, bool afectedByScale,
    ...
    public struct FlattenAtPointsTextureJob : IJobFor
    {
        ...
        public void Execute(int i)
        {
            ...
            for(int p = 0; p < PointsCount; p++){
                ...
                {
                    int2 indices = (int2)math.ceil(uv*textureIndex.Resolution);
                    int index = MapTools.VectorToIndex(indices, textureIndex.Resolution);                    
                    // Apply smooth falloff and texture value
                    float textureValue = textureMap[index+textureIndex.IndexOffset];
        ...
        public static JobHandle ScheduleParallel(NativeArray<float> globalMap, NativeArray<float> textureMap, NativeArray<float3> vertices, NativeArray<PointTransform> points,
            IndexAndResolution target, IndexAndResolution current, IndexAndResolution textureIndex,
            int PointsCount, int VerticesResolution, float Distance, 
