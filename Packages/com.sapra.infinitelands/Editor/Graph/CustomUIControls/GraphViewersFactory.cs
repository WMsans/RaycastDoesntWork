...
namespace sapra.InfiniteLands.Editor{
    ...
    {
        #region Nodes
        public static NodeView CreateNodeView(TerrainGeneratorView view, InfiniteLandsNode node)
        {
        ...
        public static OutputPreview CreateOutputPreview(PortData portData, InfiniteLandsNode node, NodeView view)
        {
            ...
            if (portData.listIndex >= 0)
            {
                if (type.IsArray)
                {
                    type = type.GetElementType(); // Get array element type
                }
                else if (type.IsGenericType && typeof(IEnumerable).IsAssignableFrom(type))
                {
                    type = type.GetGenericArguments()[0]; // Get generic type argument (List<T>)
                }
            }
            
            if(type==typeof(object)){
                type = EditorTools.GetTypeFromOutputField(portData.fieldName, node, view.graph);
            }
