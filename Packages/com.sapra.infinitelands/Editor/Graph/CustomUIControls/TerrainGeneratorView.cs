...
namespace sapra.InfiniteLands.Editor{
    ...
    public partial class TerrainGeneratorView : GraphView
    {
        ...
        {
            if (tree != null)
                tree.OnValuesChanged -= TrigerPreview;
            ...
            contextualMenu = new ContextualMenuBuilder(this);
            ReloadView();
        }
        ...
        {   
            ...
            if (nd is NodeView view)
            {
                ...
                if(nd == null){
                    ...
                    AddNode(missingView);
                    Debug.LogFormat("Missing {0}", guid);
                    return missingView;
                }
            }
        ...
        void RedrawNodes(HashSet<NodeView> nodes){
            if(nodes.Count > 0){
                tree.ValidationCheck();
        ...
        private void ExpandNodes(bool state){
            if(!state)
                HidePreviews();

            tree.nodes.ForEach(n => {
                NodeView node = FindNodeView(n.guid);
                node.TriggerExpandedState(state);
            });
        }
        ...
        {   
            ...
            if(targetNode != null){
                evt.menu.AppendAction("Open Documentation _F1", a => targetNode.OpenDocumentation());
                evt.menu.AppendSeparator();
            }

            evt.menu.AppendAction("View/Expand Nodes", a => ExpandNodes(true));
            evt.menu.AppendAction("View/Collapse Nodes", a => ExpandNodes(false));
            evt.menu.AppendSeparator("View/");
            evt.menu.AppendAction("View/Expand Preview", a => ShowPreviews());
