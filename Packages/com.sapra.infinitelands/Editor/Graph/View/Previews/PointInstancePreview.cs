...
namespace sapra.InfiniteLands.Editor
{
    ...
    public class PointInstancePreview : OutputPreview
    {
        ...
        public override VisualElement GetPreview(BranchData settings, GraphSettings graphSettings)
        {
            ...
            var foundData = Node.TryGetOutputData(settings, out PointInstance data, PortData.fieldName,PortData.listIndex);
            ...
            if (data.PreviousInstance != null)
            {
                VisualElement previous = previousVisualizer.CreateVisual(data.PreviousInstance, settings, false, graphSettings.MeshScale);
                StackVisuals(container, previous);

            }

            VisualElement current = visualizer.CreateVisual(data, settings, true, graphSettings.MeshScale);
