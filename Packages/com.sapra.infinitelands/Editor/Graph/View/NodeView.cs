...
namespace sapra.InfiniteLands.Editor{
    ...
    public class NodeView : Node, IRenderSerializableGraph
    {
        ...
        public TerrainGeneratorView treeView;
        public IGraph graph;
        ...
        public virtual void AddSelfToElements(TerrainGeneratorView view)
        {
            view.AddElementToDictionary(viewDataKey, this);
        }

        public NodeView(TerrainGeneratorView view, InfiniteLandsNode node)
        {
            this.treeView = view;
            ...
            EditorTools.ApplyClassToNodeView(this);

            hiddenProperties = new();
            hiddenProperties.style.display = EditorTools.DebugMode ? DisplayStyle.Flex : DisplayStyle.None;
        ...
        private void EnableOrDisableField(VisualElement propField, string field_name){
            object found = EditorTools.GetValueDynamic(node, field_name);
            if(found != null){
                bool active = (bool)found;
                propField.visible = active;
                if(!active){
