    ...
    internal class TerrainGeneratorEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (textureFoldoutFinal)
            {
                ...
                var textures = generator.GetAssets()
                    .OfType<InfiniteLandsAsset>() // Filter assets of type InfiniteLandsAsset
                    .Where(a => a is IHoldTextures) // Ensure the asset implements IHoldTextures
                    .SelectMany(a => ((IHoldTextures)a).GetTextures(), (asset, texture) => new { asset, texture }) // Access textures via IHoldTextures
                ...
                foreach (var texture in textures)
                {
                    ...
                    if (textureFoldouts[texture.Key])
                    {
                        ...
                        {
                            EditorGUILayout.ObjectField(refre.asset, typeof(InfiniteLandsAsset), false);
                        }
                        EditorGUI.EndDisabledGroup();
